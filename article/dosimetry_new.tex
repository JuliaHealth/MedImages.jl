...
The Improved UDE was evaluated on a full-body validation cohort using a sliding-window reconstruction ($64^3$ patches, stride 32). To assess the trade-off between physical fidelity and computational cost, we benchmarked all models on a representative full-body SPECT/CT volume ($256 \times 256 \times 345$). Table~\ref{tab:dosimetry_comparison} summarizes both the predictive accuracy and the full-body inference execution time.
...
\caption{Full-Body Performance and Computational Efficiency Comparison}
\label{tab:dosimetry_comparison}
\begin{tabular}{lccc}
\hline
\textbf{Model} & \textbf{Mean Pearson ($r$)} & \textbf{MAE (Gy)} & \textbf{Inference (s)} \\ \hline
Analytical Baseline (Static) & 0.8249 & 700.43 & \textbf{0.59} \\
DblurDoseNet (PyTorch) & 0.5566 & 1102.34 & 7.76 \\
CNN Improved (Lux/Julia) & 0.9434 & 485.20 & 8.50 \\
Spect0Net (PyTorch) & 0.6124 & 955.12 & 8.12 \\
SemiDose (PyTorch) & 0.6401 & 912.88 & 7.95 \\
Original UDE (2-Branch) & 0.9031 & 405.77 & 85.42 \\
\textbf{UDE IMPROVED (Triple-Branch)} & \textbf{0.9221} & \textbf{359.49} & \textbf{100.87} \\ \hline
\end{tabular}
\end{table}

Quantitative analysis reveals that while purely data-driven CNN architectures (e.g., DblurDoseNet, CNN Improved) offer significant speed advantages due to their feed-forward nature, they lack the physical constraints necessary for state-of-the-art accuracy. The UDE IMPROVED model, while more computationally intensive ($\sim 101$s for full-body inference), provides the highest clinical fidelity by explicitly integrating mechanistic dose deposition laws.
