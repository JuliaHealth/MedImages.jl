...
function build_cnn_with_approx(width::Int, depth::Int)
...
function train_model(mode::String, max_epochs::Int=500, patience::Int=50)
    println("\n>>> DEBUG TRAINING MODE: $mode")
    ...
    width, depth = 32, 3
    model = (mode == "CNN_APPROX") ? build_cnn_with_approx(width, depth) : build_ude_no_approx(width, depth)
    ...
    for epoch in 1:max_epochs
        ...
        for pat in train_pats
            ...
            for patch_idx in 1:1
                idx = rand(valid); cx, cy, cz = idx.I; p_s = 32; hx = p_s ÷ 2
                ...
                den_raw_dev = dev(Float32.(den_raw)) # Ensure this is a CuArray

                loss_val, gs = Zygote.withgradient(t -> begin
                    if mode == "CNN_APPROX"
                        input = reshape(stack([sp_std, den_p, app_std]), p_s, p_s, p_s, 3, 1)
                        ...
                        function f(u,p,time)
                            ...
                            in_nn = (reshape(A_t_std, p_s, p_s, p_s, 1, 1), reshape(den_p, p_s, p_s, p_s, 1, 1))
                        ...
                        sol = solve(prob, VCABM(), saveat=[300.0f0], reltol=1f-3, abstol=1f-3, sensealg=InterpolatingAdjoint(autojacvec=ZygoteVJP()))
                        pred_r = (sol.u[end].DOSE .- minimum(sol.u[end].DOSE)) ./ (maximum(sol.u[end].DOSE) + 1f-6)
                    end
                    mae = sum(abs.(pred_r .- target_p)) / 32768
                    mae + 0.3f0 * gradient_loss(pred_r, target_p)
                end, θ)
                if !any(isnan.(gs[1])); opt_state, θ = Optimisers.update(opt_state, θ, gs[1]); epoch_loss += loss_val; count += 1; end
                CUDA.reclaim(); GC.gc()
            end
        end
        avg_l = epoch_loss/count
        if avg_l < best_loss; best_loss = avg_l; epochs_without_impr = 0; serialize("elsarticle/dosimetry/model_best_$(mode).jls", θ) else epochs_without_impr += 1 end
...
train_model("CNN_APPROX", 500, 50)
train_model("UDE_NO_APPROX", 500, 50)
