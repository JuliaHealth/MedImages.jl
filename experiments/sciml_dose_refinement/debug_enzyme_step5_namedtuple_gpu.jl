...
function debug_enzyme_namedtuple_gpu()
    println("--- Step 5: Dense NN on GPU with NamedTuple (No ComponentArrays) ---")
    dev = Lux.gpu_device()
    rng = Random.default_rng()
    
    model = Chain(Dense(1 => 4, tanh), Dense(4 => 1))
    ps, st = Lux.setup(rng, model)
    θ = dev(ps) # Keep as NamedTuple!
    st = dev(st)
    ...
    println("Testing Enzyme VJP backward pass on GPU (NamedTuple)...")
    try
        loss, gs = Zygote.withgradient(loss_fn, θ)
        println("Enzyme Loss: ", loss)
        println("Gradients computed successfully! Norm: ", sum(abs2, Optimisers.destructure(gs[1])[1]))
    catch e
