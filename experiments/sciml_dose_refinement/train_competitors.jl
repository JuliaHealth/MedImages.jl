...
# --- GroupNorm Stabilized Architecture ---

function ResBlockNorm(channels::Int)
    # GroupNorm is perfect for batch_size=1
    return SkipConnection(
        Chain(
            Conv((3, 3, 3), channels => channels, pad=1),
            GroupNorm(channels, 8, relu),
            Conv((3, 3, 3), channels => channels, pad=1),
            GroupNorm(channels, 8)
        ),
        +
    )
end
...
function huber_loss(x, y, δ=0.1f0)
    diff = abs.(x .- y)
    loss = ifelse.(diff .<= δ, 0.5f0 .* diff.^2, δ .* (diff .- 0.5f0 .* δ))
...
function train_stabilized_cnn(max_epochs::Int=500, patience::Int=50)
    println("\n>>> TRAINING PHASE 4: STABILIZED RESNET (GroupNorm, Huber Loss)")
    ...
    width, depth = 32, 3
    model = build_stabilized_cnn(width, depth)
    ...
    for epoch in 1:max_epochs
        ...
        for pat in train_pats
            ...
            for patch_idx in 1:1
                ...
                den_std = dev(Float32.(standardize(hu_to_density.(ct_i[xr,yr,zr]))))
                ...
                loss_val, gs = Zygote.withgradient(t -> begin
                    ...
                    h_l = huber_loss(pred_r, target_p, 0.1f0)
                    g_l = gradient_loss(pred_r, target_p)
                    0.7f0 * h_l + 0.3f0 * g_l
                end, θ)
        ...
        if avg_l < best_loss; best_loss = avg_l; epochs_without_impr = 0; serialize("elsarticle/dosimetry/model_best_CNN_APPROX.jls", θ) else epochs_without_impr += 1 end
