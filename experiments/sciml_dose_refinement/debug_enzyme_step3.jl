...
using DifferentialEquations, Lux, LuxCUDA, CUDA, Random, ComponentArrays, SciMLSensitivity, Optimisers, Zygote, Enzyme

function build_minimal_cnn_3d()
    return Chain(Conv((3, 3, 3), 1 => 2, tanh, pad=1), Conv((3, 3, 3), 2 => 1, pad=1))
end

function debug_enzyme_step3()
    println("--- Step 3: Small 3D Neural ODE on GPU ---")
    dev = Lux.gpu_device()
    rng = Random.default_rng()
    
    p_s = 8 # Small 3D patch
    ...
    println("Testing Enzyme VJP backward pass on GPU...")
    try
