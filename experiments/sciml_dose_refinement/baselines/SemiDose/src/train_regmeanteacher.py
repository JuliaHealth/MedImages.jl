'''
-----------------------------------------------
File Name: train_regmeanteacher.py$
Description: 
  ...
  method: regression meanteacher with/without pseudo label
Author: Jing Zhang
Date: 18/09/2024
-----------------------------------------------
'''
...
if __name__ == "__main__":
    '''
    train regmeanteacher 
    '''
    
    message_start = f'baseline model: {baseline}, regmeanteacher\n'
            ...
            for labeled_data, unlabeled_data in zip(labeled_loader, unlabeled_loader):
                ...
                with torch.enable_grad():
                    ...
                    if unlabeled_data is not None:
                        ...
                        p2_w = stu_model(x2_w) 
                        p2_w = torch.squeeze(p2_w,dim=1)
                    
                        with torch.no_grad():
                            if epoch < MeanTEpoch: 
                                cons_loss1,cons_loss2 = 0,0
                                l3 = 0
                                ...
                                if labeled_data is not None: 
                                    p1_s = tea_model(x1_s)
                                    p1_s = torch.squeeze(p1_s,dim=1)
                                    cons_loss1 = F.l1_loss(p1_w, p1_s)
                                p2_s = tea_model(x2_s)
                                p2_s = torch.squeeze(p2_s,dim=1)
                                cons_loss2 = F.l1_loss(p2_w, p2_s)
                    ...
                    total_loss = l1 + alpha1 * consistency_weight * (cons_loss1+cons_loss2)+ alpha2 * l3 / n_ps  # total loss for a batch
                    total_loss.backward()
                    optimizer.step()
