...
function run_eval_all()
    ...
    θ = isfile(cp) ? adapt(Lux.CPUDevice(), deserialize(cp)) : nothing
    m = build_ude_improved(32, 3); _, st = Lux.setup(Random.default_rng(), m)
    ...
    for pat in val_cases
        ...
        ct_p = ct_i[xr, yr, zr]; sp_p = sp_i[xr, yr, zr]; mc_p = mc_i[xr, yr, zr]
        
        # Analytical Baseline
        den = hu_to_den.(ct_p)
        pred_base = (Float32.(sp_p) .* 8.478f-8) ./ (vol_p .* den .+ 1f-4)
        push!(bias_base_list, mean(pred_base .- mc_p))

        # UDE Improved
        if θ !== nothing
            pred_ude = predict_ude_patch_balanced(m, θ, st, sp_p, den, vol_p)
            push!(bias_ude_list, mean(pred_ude .- mc_p))
        end
        println("Processed $pat")
    end

    println("\nMean Centered Error (Bias) [mGy]:")
    println("Analytical Baseline: ", round(mean(bias_base_list) * 1000, digits=2))
    if !isempty(bias_ude_list)
        println("UDE IMPROVED:        ", round(mean(bias_ude_list) * 1000, digits=2))
    end
end
